...
def _make_thd_fused_inputs(
    ...
    """Build ragged packed inputs with per-sequence ``[original, compressed]`` KV."""
    torch.manual_seed(seed)
    comp_lens = tuple(q_len // ratio for q_len in q_lens)
    cu_q = torch.tensor(
    ...
    cu_comp = torch.tensor(
        [0] + list(torch.tensor(comp_lens).cumsum(0).tolist()),
        dtype=torch.int32,
        device=device,
    )
    full_lens = tuple(q_len + comp_len for q_len, comp_len in zip(q_lens, comp_lens))
    cu_full = torch.tensor(
        [0] + list(torch.tensor(full_lens).cumsum(0).tolist()),
        dtype=torch.int32,
        device=device,
    )

    total_q = sum(q_lens)
    query = torch.randn(total_q, np_, d, device=device, dtype=torch.bfloat16)
    q_indexer = torch.randn(total_q, idx_nh, idx_d, device=device, dtype=torch.bfloat16)
    weights = torch.randn(total_q, idx_nh, device=device, dtype=torch.bfloat16).abs()
    k_indexer = torch.randn(sum(comp_lens), idx_d, device=device, dtype=torch.bfloat16)

    kv_segments = []
    compressed_segments = []
    window_segments = []
    for q_len, comp_len in zip(q_lens, comp_lens):
        original = torch.randn(q_len, d, device=device, dtype=torch.bfloat16)
        compressed = torch.randn(comp_len, d, device=device, dtype=torch.bfloat16)
        kv_segments.append(torch.cat((original, compressed), dim=0))
        compressed_segments.append(compressed)
        window_segments.append(get_window_topk_idxs(win_topk, 1, q_len, device).squeeze(0))

    return {
        "q_lens": q_lens,
        "comp_lens": comp_lens,
        "query": query,
        "kv_full": torch.cat(kv_segments, dim=0),
        "compressed_kv": torch.cat(compressed_segments, dim=0),
        "attn_sink": torch.randn(np_, device=device, dtype=torch.float32) * 0.1,
        "window_idxs": torch.cat(window_segments, dim=0).int(),
        "q_indexer": q_indexer,
        "k_indexer": k_indexer,
        "weights": weights,
        "cu_q": cu_q,
        ...
        "indexer_topk": indexer_topk,
        "softmax_scale": d**-0.5,
        "indexer_softmax_scale": idx_d**-0.5,
    }
...
# ---------------------------------------------------------------------------
# THD packed-sequence accuracy tests
# ---------------------------------------------------------------------------


@_skip_unless_sm90
class TestDSAKernelTHD:
    """Validate ragged THD layout, sequence isolation, and backward parity."""

    def test_sparse_attention_multisegment_matches_unfused(self):
        torch.manual_seed(20260815)
        device = torch.device("cuda")
        q_lens = (19, 11, 23)
        kv_lens = (27, 17, 31)
        num_heads, head_dim, topk = 16, 64, 8
        cu_q = torch.tensor(
            [0] + list(torch.tensor(q_lens).cumsum(0).tolist()),
            dtype=torch.int32,
            device=device,
        ...
        query_base = torch.randn(
            sum(q_lens), num_heads, head_dim, device=device, dtype=torch.bfloat16
        )
        kv_base = torch.randn(sum(kv_lens), head_dim, device=device, dtype=torch.bfloat16)
        sink_base = torch.randn(num_heads, device=device, dtype=torch.float32) * 0.1
        local_index_segments = []
        for q_len, kv_len in zip(q_lens, kv_lens):
            local = torch.randint(
                0, kv_len, (q_len, topk), device=device, dtype=torch.int32
            )
            local[torch.rand(q_len, topk, device=device) < 0.15] = -1
            local[:, 0] = 0
            local_index_segments.append(local)
        local_indices = torch.cat(local_index_segments, dim=0)
        ...
        query_thd = query_base.clone().requires_grad_(True)
        kv_thd = kv_base.clone().requires_grad_(True)
        sink_thd = sink_base.clone().requires_grad_(True)
        output_thd = _triton_dsa_sparse_attn_sbhd(
            query_thd,
            kv_thd,
            sink_thd,
            flat_indices,
        ...
        query_ref = query_base.clone().requires_grad_(True)
        kv_ref = kv_base.clone().requires_grad_(True)
        sink_ref = sink_base.clone().requires_grad_(True)
        output_segments = []
        for segment, (q_len, kv_len) in enumerate(zip(q_lens, kv_lens)):
            q_start, q_end = cu_q[segment].item(), cu_q[segment + 1].item()
            kv_start, kv_end = cu_kv[segment].item(), cu_kv[segment + 1].item()
            output_segments.append(
                unfused_compressed_sparse_attn(
                    query_ref[q_start:q_end].unsqueeze(1),
                    kv_ref[kv_start:kv_end].unsqueeze(1),
                    sink_ref,
                    local_index_segments[segment].unsqueeze(0),
                    head_dim**-0.5,
                ).squeeze(1)
            )
        output_ref = torch.cat(output_segments, dim=0)

        grad_output = torch.randn_like(output_thd)
        output_thd.backward(grad_output)
        output_ref.backward(grad_output)
        torch.testing.assert_close(output_thd, output_ref, rtol=5e-2, atol=5e-2)
        torch.testing.assert_close(query_thd.grad, query_ref.grad, rtol=2e-2, atol=2e-2)
        torch.testing.assert_close(kv_thd.grad, kv_ref.grad, rtol=2e-2, atol=2e-2)
        torch.testing.assert_close(sink_thd.grad, sink_ref.grad, rtol=2e-2, atol=2e-2)

    @pytest.mark.parametrize("sparse_loss", [True, False], ids=["sparse_loss", "dense_loss"])
    def test_fused_indexer_multisegment_matches_per_segment_reference(self, sparse_loss):
        inputs = _make_thd_fused_inputs()
        ...
        thd = {
            name: inputs[name].clone().requires_grad_(True) for name in differentiable_names
        }
        output_thd, loss_thd = fused_indexer_sparse_attn(
            thd["query"],
            thd["kv_full"],
            thd["attn_sink"],
            inputs["window_idxs"],
            ...
            cu_seqlens_kv=inputs["cu_kv"],
            cu_seqlens_kv_full=inputs["cu_full"],
            cu_seqlens_compressed_idx=inputs["cu_comp"],
            max_seqlen_q=inputs["max_q"],
            max_seqlen_compressed_idx=inputs["max_comp"],
        ...
        output_segments = []
        weighted_losses = []
        total_q = inputs["query"].shape[0]
        for segment, q_len in enumerate(inputs["q_lens"]):
            q_start = inputs["cu_q"][segment].item()
            q_end = inputs["cu_q"][segment + 1].item()
            comp_start = inputs["cu_comp"][segment].item()
            comp_end = inputs["cu_comp"][segment + 1].item()
            full_start = inputs["cu_full"][segment].item()
            full_end = inputs["cu_full"][segment + 1].item()
            output_segment, loss_segment = fused_indexer_sparse_attn(
                ref["query"][q_start:q_end].unsqueeze(1),
                ref["kv_full"][full_start:full_end].unsqueeze(1),
                ref["attn_sink"],
                inputs["window_idxs"][q_start:q_end].unsqueeze(0),
                ref["q_indexer"][q_start:q_end].unsqueeze(1),
                ref["k_indexer"][comp_start:comp_end].unsqueeze(1),
                ref["weights"][q_start:q_end].unsqueeze(1),
                inputs["indexer_topk"],
                inputs["ratio"],
                inputs["softmax_scale"],
                inputs["indexer_softmax_scale"],
            ...
            output_segments.append(output_segment.squeeze(1))
            weighted_losses.append(loss_segment * (q_len / total_q))
        output_ref = torch.cat(output_segments, dim=0)
        loss_ref = torch.stack(weighted_losses).sum()

        grad_output = torch.randn_like(output_thd)
        (output_thd * grad_output).sum().add(loss_thd).backward()
        (output_ref * grad_output).sum().add(loss_ref).backward()
        torch.testing.assert_close(output_thd, output_ref, rtol=5e-2, atol=5e-2)
        torch.testing.assert_close(loss_thd, loss_ref, rtol=2e-3, atol=2e-4)
        for name in differentiable_names:
            torch.testing.assert_close(
                ...
                rtol=2e-2,
                atol=2e-2,
                msg=f"THD {name} gradient does not match per-segment reference",
            )
