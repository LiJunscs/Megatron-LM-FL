...
class TestDistributedTPCorrectness:
    ...
    @staticmethod
    def _run_fused_thd(inputs, query_base, sink_base, tp_group, sparse_loss):
        """Run ragged THD fused DSA for either full or TP-local query heads."""
        import megatron.plugin.dsa_kernel.triton_dsa_kernels as _mod

        _mod._DSA_TP_OVERLAP = False
        query = query_base.clone().requires_grad_(True)
        kv_full = inputs["kv_full"].clone().requires_grad_(True)
        sink = sink_base.clone().requires_grad_(True)
        q_indexer = inputs["q_indexer"].clone().requires_grad_(True)
        k_indexer = inputs["k_indexer"].clone().requires_grad_(True)
        weights = inputs["weights"].clone().requires_grad_(True)
        output, loss = fused_indexer_sparse_attn(
    ...
    def test_fused_thd_tp_matches_tp1(self, sparse_loss):
        """Ragged THD TP output, loss, and indexer gradients match TP1."""
        from tests.unit_tests.plugin.dsa_kernel.test_fused_dsa import (
            _make_thd_fused_inputs,
        )

        rank = self._rank
        tp_group = self._tp_group
        tp_size = self._tp_size
        num_heads = max(16, tp_size * 4)
        inputs = _make_thd_fused_inputs(np_=num_heads)
        local_heads = num_heads // tp_size
        head_start = rank * local_heads
        head_end = head_start + local_heads

        reference = self._run_fused_thd(
            inputs,
            inputs["query"],
            inputs["attn_sink"],
            None,
        ...
        tp_result = self._run_fused_thd(
            inputs,
            inputs["query"][:, head_start:head_end].contiguous(),
            inputs["attn_sink"][head_start:head_end].contiguous(),
            tp_group,
            sparse_loss,
        )

        total_q, _, head_dim = inputs["query"].shape
        reference_output = reference["output"].reshape(total_q, num_heads, head_dim)
        reference_output = reference_output[:, head_start:head_end].reshape(
            total_q, local_heads * head_dim
        )
        torch.testing.assert_close(
            tp_result["output"],
            reference_output,
            rtol=1e-2,
            atol=2e-2,
            msg=f"THD TP={tp_size} output does not match TP1 head slice",
        )
        torch.testing.assert_close(
            tp_result["grad_query"],
            reference["grad_query"][:, head_start:head_end],
            rtol=1e-2,
            atol=2e-2,
            msg=f"THD TP={tp_size} query gradient does not match TP1 head slice",
        )
        torch.testing.assert_close(
            tp_result["grad_sink"],
            reference["grad_sink"][head_start:head_end],
            rtol=1e-2,
            atol=2e-2,
            msg=f"THD TP={tp_size} sink gradient does not match TP1 head slice",
        )
        torch.testing.assert_close(
            tp_result["loss"],
            reference["loss"],
            rtol=1e-4,
            atol=1e-6,
            msg=f"THD TP={tp_size} indexer loss does not match TP1",
        )
        for name in ("grad_q_indexer", "grad_k_indexer", "grad_weights"):
            torch.testing.assert_close(
                ...
                rtol=2e-2,
                atol=2e-2,
                msg=f"THD TP={tp_size} replicated {name} does not match TP1",
            )
