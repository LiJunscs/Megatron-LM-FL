...
Optimized to avoid materializing O(S_q 脳 S_k) intermediates where possible:
...
def sparse_indexer_score_recompute(
    ...
    Args:
        ...
        weights:   ``(B, S_q, H_q)`` bf16 鈥?scaled weights (w_raw * indexer_softmax_scale).
    ...
    # Gather K at topk positions using batch advanced indexing 鈥?O(B*S_q*topk*D)
    # No S_q脳S_k expansion needed.
    ...
    # Compute scores: (B, S_q, H_q, topk) = Q @ K^T (no scale 鈥?embedded in weights)
...
def sparse_attn_score_recompute(
    ...
    Args:
        ...
        k_attn: ``(B, S_k, D)`` bf16.
        lse:    ``(B, S_q, H_q)`` fp32 鈥?log-sum-exp from attention forward.
        topk_indices: ``(B, S_q, topk)`` int32.
    ...
    # Gather K at topk positions 鈥?O(B*S_q*topk*D), no S_k expansion
    idx_expanded = topk_indices.long().clamp(min=0)  # (B, S_q, topk)
...
# ---------------------------------------------------------------------------
# Dense indexer score: full S_k scoring 鈥?tiled over Q blocks
# ---------------------------------------------------------------------------
...
def dense_indexer_score_recompute(
    ...
    # Pre-compute ratio causal mask (only S_q 脳 S_k, typically small fp32)
...
# ---------------------------------------------------------------------------
# Dense attention score: full S_k scoring 鈥?tiled over Q blocks
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Indexer top-K selection 鈥?tiled over Q blocks
# ---------------------------------------------------------------------------
...
def indexer_topk_selection(
    ...
    Args:
        q_indexer: ``(B, S_q, H_q, D)`` bf16.
        k_indexer: ``(B, S_k, D)`` bf16.
        weights:   ``(B, S_q, H_q)`` bf16 鈥?scaled weights.
    ...
    Returns:
        dict with:
            ...
            "full_scores": ``(B, S_q, S_k)`` fp32 鈥?full indexer scores (for backward).
    """
    B, S_q, H_q, D = q_indexer.shape
    S_k = k_indexer.shape[1]
...
# ---------------------------------------------------------------------------
# Indexer backward 鈥?sparse path
# ---------------------------------------------------------------------------
...
def sparse_indexer_backward(
    ...
    The forward computes:
        scores[h,t] = ReLU(Q[h] 路 K[t]^T) * scale
    ...
    # Gather K at topk positions 鈥?batch advanced indexing, no S_k expansion
    idx_expanded = topk_indices.long().clamp(min=0)  # (B, S_q, topk)
    ...
    # Scatter grad_k_gathered back to full k_indexer 鈥?vectorized, no Python loop
...
# ---------------------------------------------------------------------------
# Indexer backward 鈥?dense path (tiled over Q blocks)
# ---------------------------------------------------------------------------
...
def dense_indexer_backward(
    ...
    Args:
        ...
        k_indexer: ``(B, S_k, D)`` bf16.
        weights: ``(B, S_q, H_q)`` bf16.
        indexer_out: ``(B, S_q, S_k)`` fp32 鈥?raw indexer scores.
        indexer_denom: ``(B, S_q)`` fp32 鈥?LSE of indexer_out.
        attn_out: ``(B, S_q, S_k)`` fp32 鈥?head-summed attention weights.
        attn_denom: ``(B, S_q)`` fp32 鈥?L1 norm of attn_out.
        qhead_per_kv_head: MQA ratio.
        indexer_softmax_scale: scale factor.
    ...
    # Recompute predict and target (these are (B, S_q, S_k) 鈥?already stored)
    ...
    # Backward through indexer score computation 鈥?tiled over Q blocks
    # indexer_out[b, q, k] = sum_h(ReLU(Q[b,q,h] 路 K[b,k]^T * scale) * W[b,q,h])
    k = k_indexer.float()  # (B, S_k, D)
...
def compute_sparse_local_target_head_sum(
    ...
    Args:
        q_attn_bshd: ``(B, S_q, np, D_attn)`` bf16 鈥?attention queries.
        k_attn_bsd: ``(B, S_kv, D_attn)`` bf16 鈥?full attention keys.
        lse_bsh: ``(B, S_q, np)`` fp32 鈥?LSE from sparse attention forward.
        topk_indices_cmp: ``(B, S_q, topk)`` int32 鈥?indices into [0, n_comp).
        softmax_scale: scale for attention scores.
        kv_offset: offset where compressed KV starts in k_attn_bsd.

    Returns:
        head_sum: ``(B, S_q, topk)`` fp32 鈥?sum of attention probs over local heads.
    """
...
def compute_sparse_indexer_predict_state(
    ...
    Args:
        q_idx_bshd: ``(B, S_q, H_q, D_idx)`` bf16 鈥?indexer queries.
        k_idx_bsd: ``(B, S_k, D_idx)`` bf16 鈥?indexer keys.
        w_bsh: ``(B, S_q, H_q)`` bf16 鈥?scaled weights.
        topk_indices_cmp: ``(B, S_q, topk)`` int32 鈥?indices into [0, n_comp).
...
def sparse_indexer_kl_and_backward(
    ...
    Args:
        head_sum: ``(B, S_q, topk)`` fp32 鈥?**already all-reduced** global head sum.
        predict_state: dict from ``compute_sparse_indexer_predict_state``.
        q_idx_bshd: ``(B, S_q, H_q, D_idx)`` 鈥?for grad dtype.
        k_idx_bsd: ``(B, S_k, D_idx)`` 鈥?for grad shape/dtype.
        w_bsh: ``(B, S_q, H_q)`` 鈥?for grad dtype.
...
def fused_sparse_indexer_loss_and_backward(
    ...
    Args:
        q_idx_bshd: ``(B, S_q, H_q, D_idx)`` bf16 鈥?indexer queries.
        k_idx_bsd: ``(B, S_k, D_idx)`` bf16 鈥?indexer keys.
        w_bsh: ``(B, S_q, H_q)`` bf16 鈥?scaled weights (w_raw * indexer_softmax_scale).
        topk_indices_cmp: ``(B, S_q, topk)`` int32 鈥?indices into [0, n_comp).
        q_attn_bshd: ``(B, S_q, np, D_attn)`` bf16 鈥?attention queries.
        k_attn_bsd: ``(B, S_kv, D_attn)`` bf16 鈥?attention keys (full KV buffer).
        lse_bsh: ``(B, S_q, np)`` fp32 鈥?LSE from attention forward.
        indexer_softmax_scale: scale for indexer scores.
        softmax_scale: scale for attention scores.
        loss_coeff: KL loss coefficient.
        calculate_per_token_loss: if True, use sum instead of mean.
...
def fused_dense_indexer_loss_and_backward(
    ...
    q_attn_bshd: Tensor,
    k_attn_bsd: Tensor,
    lse_bsh: Tensor,  # noqa: ARG001 鈥?kept for API compat; dense uses self-contained softmax
    indexer_softmax_scale: float,  # noqa: ARG001 鈥?scale is embedded in w_bsh; kept for API compat
    softmax_scale: float,
    loss_coeff: float,
    ratio: int = 1,
    calculate_per_token_loss: bool = False,
    ...
    Args:
        q_idx_bshd: ``(B, S_q, H_q, D_idx)`` bf16 鈥?indexer queries.
        k_idx_bsd: ``(B, S_k, D_idx)`` bf16 鈥?indexer keys (compressed range).
        w_bsh: ``(B, S_q, H_q)`` bf16 鈥?raw weights.
        topk_indices_cmp: ``(B, S_q, topk)`` int32 鈥?for row validity.
        q_attn_bshd: ``(B, S_q, np, D_attn)`` bf16 鈥?attention queries.
        k_attn_bsd: ``(B, S_k, D_attn)`` bf16 鈥?attention keys (compressed range).
        lse_bsh: ``(B, S_q, np)`` fp32 鈥?LSE from attention forward.
        indexer_softmax_scale: scale for indexer scores.
        softmax_scale: scale for attention scores.
        loss_coeff: KL loss coefficient.
    ...
    for q_start in range(0, S_q, BLOCK_Q):
        ...
        # Grad through matmul: score = Q@K (no scale 鈥?embedded in weights)
