...
def topk_with_causal_mask(
    ...
    Args:
        scores: ``(B, S_q, S_k)`` float32 鈥?raw indexer scores.
        k: number of top-K indices to select.
        ratio: compression ratio for causal masking.

    Returns:
        topk_indices: ``(B, S_q, k)`` int32 鈥?indices into S_k, -1 for invalid.
        topk_length: ``(B, S_q)`` int32 鈥?number of valid positions per query.
    """
    ...
    # Effective k 鈥?cannot exceed S_k
...
def select_block_sizes(D: int, TopK: int) -> Tuple[int, int]:
    ...
    """
    if D >= 512:
        # Large head dim 鈥?use smaller blocks
        BLOCK_Q = 16
