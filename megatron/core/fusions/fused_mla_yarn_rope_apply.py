...
def _mla_rope_fwd_inplace_kernel(
    ...
    if position_ids is not None:
        position_row = pid_m // batch_size if cu_seqlens_q is None else pid_m
        token_idx = tl.load(position_ids + position_row)
...
class _FusedMLARoPEInplace(torch.autograd.Function):
    ...
    def forward(
        ...
        if cu_seqlens_q is None:
            ...
            if position_ids is not None:
                assert position_ids.shape == (max_seqlen,)
...
def fused_mla_rope_inplace(
    ...
    Args:
        ...
        position_ids: optional explicit position for every sequence row. For SBHD,
            the same ``[seq_len]`` positions are shared across the batch. When
            omitted, the existing SBHD/THD CP mapping is used.
